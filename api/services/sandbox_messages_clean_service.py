...
class SandboxMessagesCleanService:
    ...
    def clean_sandbox_messages(cls) -> dict[str, int]:
        ...
        while True:
            ...
            sandbox_tenant_ids = [
                tenant_id for tenant_id in tenant_ids if tenant_plans.get(tenant_id) == CloudPlan.SANDBOX
            ]
            ...
            while True:
                with Session(db.engine, expire_on_commit=False) as session:
                    stmt = (
                        select(Message.id)
                        .where(Message.app_id.in_(app_ids), Message.created_at < plan_sandbox_clean_message_day)
                        .limit(dify_config.SANDBOX_MESSAGES_CLEAN_MESSAGE_BATCH_SIZE)
                    )
                    message_ids = list(session.scalars(stmt).all())

                    if not message_ids:
                        logger.info("clean_messages (batch %s): no more messages, end this batch", stats["batches"])
                        break
